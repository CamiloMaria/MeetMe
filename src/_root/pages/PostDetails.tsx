...
const PostDetails = () => {
	...
	const { data: userPosts, isLoading: isUserPostLoading } = useGetUserPosts(post?.userId);
	const { mutate: deletePost } = useDeletePost();

	const relatedPosts = userPosts?.filter((p) => p.postId !== id);
				...
				<div className="post_details-card">
					...
					<div className="post_details-info">
						<div className="flex-between w-full">
							<Link to={`/profile/${user.accountId}`} className="flex items-center gap-3">
								<img
									src={user?.imageUrl || "/assets/images/profile.png"}
									className="rounded-full w-8 lg:h-12 lg:w-12 h-12"
								/>
							...
							<div className="flex-center gap-4">
								<Button
									...
									variant="ghost"
									className={`ghost_details-delete_btn ${post?.userId !== user.accountId && "hidden"}`}
								>
